...
int main(int argc, char* argv[])
{
  ...
  // Maxiumum time offset in number of samples
  int max_time_offset = std::atof(argv[4]) * sample_rate * 2;

  double power = 0;
  double max_power = std::stof(argv[5]);

  std::cout << "Max time offset: " << max_time_offset << " samples" << std::endl;
  ...
  while (input.good())
  {
    ...
    if (power == max_power)
    {
      ...
      {
        // Increase time offset linearly a 0.2 s/min
        time_offset += 1 / (2 * 60.0 * 5);
